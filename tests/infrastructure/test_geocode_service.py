...
class TestGeocodingService:
    ...
    @pytest.mark.asyncio
    async def test_geocode_single_address_success(self, geocoding_service, mock_httpx_client):
        ...
        result = await geocoding_service._geocode_single_address("test_id", "Paris, France")
    ...
    @pytest.mark.parametrize("mock_json,expected", [
        ( {"features": []}, None ),
        ( {"features": [{"geometry": {"coordinates": [2.3522]}, "properties": {"score": 0.95}}]}, None ),
        ( {"features": [{"properties": {"score": 0.95}}]}, None ),
        ( {"features": [{"geometry": {}, "properties": {"score": 0.95}}]}, None ),
    ])
    async def test_geocode_single_address_various_no_results(self, geocoding_service, mock_httpx_client, mock_json, expected):
        """Test geocoding with no features, invalid/missing coordinates, or missing geometry."""
        mock_response = MagicMock()
        mock_response.json.return_value = mock_json
        mock_httpx_client.get.return_value = mock_response
        result = await geocoding_service._geocode_single_address("test_id", "Any Address")
    ...
    @pytest.mark.parametrize("side_effect", [
        ...
        httpx.HTTPStatusError("Internal Server Error", request=MagicMock(), response=MagicMock()),
        httpx.RequestError("Request failed"),
        ValueError("Invalid JSON"),
        Exception("Unexpected error")
    ])
    async def test_geocode_single_address_errors_return_none(self, geocoding_service, mock_httpx_client, side_effect):
    ...
    async def test_geocode_single_address_custom_base_url(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            result = await custom_service._geocode_single_address("test_id", "Paris, France")
            assert result == {"latitude": 48.8566, "longitude": 2.3522}
    ...
    @pytest.mark.asyncio
    async def test_geocode_addresses_multiple_addresses_success(self, geocoding_service):
    ...
    @pytest.mark.parametrize("side_effect,expected", [
        ([{"latitude": 48.8566, "longitude": 2.3522}, None, {"latitude": 45.7640, "longitude": 4.8357}], 2),
        ([{"latitude": 48.8566, "longitude": 2.3522}, GeocodingError("Connection failed"), {"latitude": 45.7640, "longitude": 4.8357}], 2),
        ([None, None], 0),
        ([GeocodingError("Connection failed"), GeocodingError("Timeout")], 0),
        ([TimeoutError("Global timeout")], 0),
        ([Exception("Critical error")], 0),
    ])
    async def test_geocode_addresses_various_results(self, geocoding_service, side_effect, expected):
    ...
    async def test_geocode_addresses_concurrent_execution(self, geocoding_service):
        ...
        with patch.object(geocoding_service, '_geocode_single_address', side_effect=mock_geocode_with_delay):
            result = await geocoding_service.geocode_addresses(addresses)
    ...
    async def test_geocode_addresses_empty_strings(self, geocoding_service):
        ...
        with patch.object(geocoding_service, '_geocode_single_address') as mock_geocode:
            mock_geocode.side_effect = [None, None, {"latitude": 48.8566, "longitude": 2.3522}]
